...
Biến đổi dữ liệu từ hình học sang dữ liệu ngôn ngữ (Tokenization)
 Tức là, mỗi landmarks cho chúng ta một vector như sau $$v = \begin{bmatrix} x \\ y \\ z \\ \end{bmatrix}$$ 
 Từ đây ta suy ra được rằng một frame Mediapipe sẽ trả về một ma trận
$$X = \begin{bmatrix}v_1 \\v_2 \\ \vdots \\v_n \end{bmatrix}$$
 Từ đây ta suy ra được rằng số chiều của ma trận này là một ma trận có số chiều là $$ X \in \mathbb{R}^{63}$$
 Tiếp theo, chúng ta sẽ xây dựng một bộ từ điển cho mô hình LSTM. Với $ k = 32 $ với mỗi vector đặc trưng $v_i$ của một tư thế tay, thuật toán tính khoảng cách Euclid đến tất cả các tâm cụm: $$ d(x_i, \mu_j) = \sqrt{\sum_{d=1}^{63} (x_{id} - \mu_{jd})^2} $$
 Mỗi tư thế tay sẽ được gán vào cụm có khoảng cách ngắn nhất.
 Sau khi gán xong, mỗi tâm cụm $\mu_j$ được tính toán lại bằng cách lấy trung bình cộng của tất cả các vector đã được gán cho nó $$\mu_j = \frac{1}{|S_j|} \sum_{x_i \in S_j} x_i$$ quá trình sẽ được lặp lại cho đến khi mô hình đã hội tụ. Kết quả mong muốn cuối cùng là thu được 32 "tư thế mẫu" đại diện cho mọi tư thế tay có thể xảy ra.
 Về bản chất, K-means thực hiện nhiệm vụ: Rời rạc hóa không gian đặc trưng liên tục. Có vẻ hơi khó hiểu nhưng thực sự thì sau khi qua K-means, ma trận $$X = \begin{bmatrix}v_1 \\v_2 \\ \vdots \\v_n \end{bmatrix}  \xrightarrow{f} Y = \begin{bmatrix}0 \\ 1 \\  \vdots \\32 \end{bmatrix}$$ sẽ được gán một ID duy nhất, ví dụ như được gán với số 5 lúc này nó đóng vai trò là một "từ vựng" (token) đại diện cho một tư thế tay cụ thể (ví dụ: có thể là tư thế xòe tay). 
...
Chúng ta sẽ lấy 16 frame để dự đoán hành động, thì input lúc này $$X_{\text{raw}} =
\begin{bmatrix}
